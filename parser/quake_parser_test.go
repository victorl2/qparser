...
func TestParserQuakeGameFile(t *testing.T) {
	const mockLogData = `------------------------------------------------------------
 20:37 ------------------------------------------------------------
 ...
 26  0:00 ------------------------------------------------------------`
	...
	// Use a string reader to simulate reading from a file
	reader := strings.NewReader(mockLogData)
	scanner := bufio.NewScanner(reader)

	// Assume targetGame is -1 to parse all games
	quakeGames := createGamesFromLogFile(scanner, -1)

	if quakeGames == nil || quakeGames.GameDetails == nil {
		t.Errorf("Expected quakeGames to be non-nil")
	}

	game, exists := quakeGames.GameDetails[1]
	if !exists {
		t.Errorf("Expected 'game_1' to exist")
	}

	if game == nil {
		t.Errorf("Expected game 1 to be non-nil")
	}

	if game.TotalKills != 11 {
		t.Errorf("Expected 11 total kills, got %d", game.TotalKills)
	}
	...
	if game.Kills["Isgalamido"] != -5 {
		t.Errorf("Expected Isgalamido to have -5 kills, got %d", game.Kills["Isgalamido"])
	}

	if game.KillByMeans["MOD_TRIGGER_HURT"] != 7 {
		t.Errorf("Expected 7 kills by MOD_TRIGGER_HURT, got %d", game.KillByMeans["MOD_TRIGGER_HURT"])
	}

	if game.KillByMeans["MOD_ROCKET_SPLASH"] != 3 {
		t.Errorf("Expected 3 kills by MOD_ROCKET_SPLASH, got %d", game.KillByMeans["MOD_ROCKET_SPLASH"])
	}

	if game.KillByMeans["MOD_TRIGGER_HURT"] != 7 {
		t.Errorf("Expected 6 kills by MOD_TRIGGER_HURT, got %d", game.KillByMeans["MOD_TRIGGER_HURT"])
	}
}
